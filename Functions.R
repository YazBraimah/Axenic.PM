...
#               geom_text_repel(aes(label=replicate)) + 
                labs(title = paste(gene, " (", geneName, ")", sep = ""), subtitle = paste(cg_id, ":\n", description, sep = "")) + 
                theme_bw() + 
        ...
        } else if (grepl("CG", gene)) {
            fbgn_id <- subset(tpm.table, annotation_ID == gene)$gene_id
            description <- subset(snapshots, FBgn_ID == fbgn_id)$GeneName
            geneName <- subset(tpm.table, annotation_ID == gene)$gene_symbol
            
            p <- ggplot(subset(tpm.table, annotation_ID == gene), aes(Male, TPM, fill = Status)) +
                geom_boxplot(outlier.size = 0, width = 0.3, alpha = 0.5) +
                geom_point(pch = 21, position = position_jitterdodge(), aes(fill = Status)) + 
                facet_grid(.~Female, scales = "free_x", space = "free_x") +
#               geom_text_repel(aes(label=replicate)) + 
                labs(title = paste(gene, " (", geneName, ")", sep = ""), subtitle = paste(fbgn_id, ":\n", description, sep = "")) + 
                theme_bw() + 
                theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 13),
                    axis.text.y = element_text(size = 14),
                    strip.text = element_text(size = 14, face = "bold")) +
            ...
            p <- ggplot(subset(tpm.table, gene_symbol == gene), aes(Male, TPM, fill = Status)) +
                ...
                facet_grid(.~Female, scales = "free_x", space = "free_x") +
#               geom_text_repel(aes(label=replicate)) + 
                labs(title = paste(gene, " (", fbgn_id, ")", sep = ""), subtitle = paste(cg_id, ":\n", description, sep = "")) + 
                theme_bw() + 
                theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 13),
                    axis.text.y = element_text(size = 14),
                    strip.text = element_text(size = 14, face = "bold")) +
